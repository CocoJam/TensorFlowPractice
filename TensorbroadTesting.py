...
def neural_network_modeling (data):
    ...
    tf.summary.histogram('Weights for linear 1', hidden_layer_1['weights'])
    tf.summary.histogram('Weights for linear 2', hidden_layer_2['weights'])
    tf.summary.histogram('Weights for linear 3', hidden_layer_3['weights'])
    tf.summary.histogram('Weights for output layer', output_layer['weights'])


    tf.summary.histogram('Bias for linear 1', hidden_layer_1['biases'])
    tf.summary.histogram('Bias for linear 2', hidden_layer_2['biases'])
    tf.summary.histogram('Bias for linear 3', hidden_layer_3['biases'])
    tf.summary.histogram('Bias for output layer', output_layer['biases'])
    return output
...
def train_nn(x):
    ...
    with tf.Session() as sess:
        ...
        write = tf.summary.FileWriter("/tmp/mnist_demo/1")
        ...
        for i in range (hm_epochs):
            ...
            for _ in range(int(mnist.train.num_examples/batch_size)):
                ...
                s = sess.run(merged_summary, feed_dict={Input_data: x, Label_data: y})
                write.add_summary(s)
        ...
        save_path = saver.save(sess, "/tmp/mnist_demo/model1.ckpt")
